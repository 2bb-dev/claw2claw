...
contract Claw2ClawHook is IHooks, ReentrancyGuard {
    ...
    struct Order {
        ...
        uint256 expiry;
        bool active;
        bytes32 poolId; // C-1 fix: bind order to its pool
    }
    ...
    error ZeroAddress();
    error PoolMismatch(); // C-1 fix
    error DurationTooLong(); // L-1 fix
    error NotPendingAdmin(); // M-2 fix

    uint256 public constant MAX_ORDER_DURATION = 30 days; // L-1 fix
    uint256 public constant MAX_ITERATIONS = 100; // H-3/H-4 fix
        ...
        external onlyWhitelisted nonReentrant returns (uint256 orderId)
    {
        ...
        if (duration > MAX_ORDER_DURATION) revert DurationTooLong(); // L-1 fix
    ...
    function cancelOrder(uint256 orderId, PoolKey calldata key) external nonReentrant {
        ...
        // C-1 fix: validate PoolKey matches the order's pool
        bytes32 poolId = keccak256(abi.encode(key));
        ...
        external onlyPoolManager nonReentrant returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        for (uint256 i = 0; i < maxIter; i++) {
            ...
            // H-1 fix: specified cancels takerPays (what we took), unspecified reflects what we settled
